...
def create_triangle_with_angle(opening_deg, rotation=0):
    ...
    vertex_point = gmsh.model.occ.addPoint(
        adjacent_length * np.cos(angle_radians),
        adjacent_length * np.sin(angle_radians),
        0,
    )
    ...
    domain = gmsh.model.occ.rotate(
        [(2, triangle_surface)], *p0, *_rotation_axis, angle_radians
    )
...
def create_pacman(opening_deg, rotation=0):
    ...
    # Define the triangle surface
    triangle_surface = gmsh.model.occ.addPlaneSurface([triangle_loop])
    _entities.append(triangle_surface)
    _rotation_axis = [0, 0, 1]
    occ.rotate([(2, triangle_surface)], *p0, *_rotation_axis, -angle_radians / 2)

    disk = occ.addDisk(*p0, _r1, _r1)
    _entities.append(disk)
    disk_ext = occ.addDisk(*p0, _r2, _r2)
    _entities.append(disk_ext)

    _machine_tag = 300
    _machine = gmsh.model.occ.cut(
        [(2, disk_ext)],
        [(2, disk)],
        tag=_machine_tag,
        removeObject=True,

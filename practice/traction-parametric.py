...
def main(parameters, model="at2", storage=None):
    ...
    mesh_refinement = parameters["geometry"].get(
        "ell_lc", parameters["geometry"].get("mesh_size_factor", 3)
    )
    _lc = ell_ / mesh_refinement
...
def load_parameters(file_path, model="at2"):
    ...
    parameters["geometry"].setdefault(
        "ell_lc", parameters["geometry"].get("mesh_size_factor", 3)
    )
...
if __name__ == "__main__":
    ...
    from irrevolutions.utils.parametric import (
        parameters_vs_ell,
    ...
    base_parameters, base_signature = load_parameters(
        repo_path("test", "parameters.yml"), model=args.model
    )

    if "-s" in sys.argv:
        parameters, signature = parameters_vs_SPA_scaling(
            parameters=base_parameters, s=float(args.s)
        )
        ...
        parameters, signature = parameters_vs_n_refinement(
            parameters=base_parameters, r=int(args.n)
        )

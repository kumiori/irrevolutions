...
from irrevolutions.models import DamageElasticityModel
...
def load_atjj_parameters():
    parameters_path = repo_path("test", "atk_parameters.yml")
    if parameters_path.exists():
        ...
        with open(repo_path("test", "parameters.yml")) as f:
            parameters = yaml.load(f, Loader=yaml.FullLoader)
        parameters["model"].setdefault("k", 3.0)
        parameters["model"].setdefault("w1", 1.0)
        parameters["model"].setdefault("k_res", 0.0)
        parameters["geometry"].setdefault("ell_lc", 4)
    return parameters
...
def parameters_vs_ell(parameters=None, ell=0.1):
    if parameters is None:
        parameters = load_atjj_parameters()
...
def param_ell():
    ...
    for ell in [0.1, 0.3]:
        ...
        parameters, _ = parameters_vs_ell(parameters, ell)
...
def param_s():
    for s in [0.001, 0.01, 0.05]:
        ...
        parameters, _ = parameters_vs_SPA_scaling(parameters, s)
        pretty_parameters = json.dumps(parameters, indent=2)
        ColorPrint.print_bold(f"   {message}    ")
        ColorPrint.print_bold("===================-===============")

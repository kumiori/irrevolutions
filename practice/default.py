...
def setup_boundary_conditions(V_u, V_alpha, Lx):
    ...
    bcs_alpha = []
    bcs = {"bcs_u": bcs_u, "bcs_alpha": bcs_alpha, "u_right": u_}
...
def run_time_loop(parameters, solver, model, bcs, postprocess=None):
    """
    ...
    geometry = mesh.geometry._cpp_object if hasattr(mesh.geometry, "_cpp_object") else mesh.geometry
    _x = _cpp.fem.interpolation_coords(V_u.element, geometry, cells)
    ...
    for i_t, t in enumerate(loads):
        ...
        bcs["u_right"].interpolate(datum)
        bcs["u_right"].x.scatter_forward()
...
if __name__ == "__main__":
    ...
    parameters, signature = load_parameters(repo_path("test", "parameters.yml"))

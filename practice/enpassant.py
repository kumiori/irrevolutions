...
from irrevolutions.meshes import primitives
from dolfinx.fem import assemble_scalar, dirichletbc, locate_dofs_geometrical
import irrevolutions.algorithms.am as am
import ufl
import pyvista
import numpy as np
import irrevolutions.meshes as meshes
...
# Parameters
parameters = {
    "loading": {"min": 0.0, "max": 50, "steps": 30},
    "geometry": {
        "geom_type": "bar",
        "Lx": 100,
        "Ly": 200,
...
mesh, _, _ = gmshio.model_to_mesh(gmsh_model, MPI.COMM_WORLD, 0, 2)
...
mesh_refined_local2 = dolfinx.mesh.refine(mesh, edges)
if isinstance(mesh_refined_local2, tuple):
    mesh_refined_local2 = mesh_refined_local2[0]

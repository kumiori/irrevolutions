...
def test_viscous(parameters, storage):
    # Adapting
    # Calc. Var. (2016) 55:17
    ...
    prefix, signature = setup_ctx(parameters, storage, comm)
    ...
    element_L2 = ufl.FiniteElement("DG", mesh.ufl_cell(), degree=0)
    L2 = dolfinx.fem.FunctionSpace(mesh, element_L2)
    ...
    _mechanical_energy = model.total_energy_density(state) * dx - external_work
    total_energy = _mechanical_energy \
    ...
    for i_t, t in enumerate(loads):
        ...
        # if partial_alpha E > 0 and rate > 0 then rate-dependent
        # in this case, further split time step
        # and flow along s.        
        ...
        assemble_vector(_slope.vector, form(_F))
        _newton_norm_r = hybrid.getReducedNorm()[0]
        
        print('reduced norms ', np.array_str(np.array(_newton_norm_r), precision=2, suppress_small=True))
        tol = 1.e-2
        if len(_slope.vector[ _slope.vector[:] > tol ]) and rate_12_norm > 0:
            __import__('pdb').set_trace()
            ColorPrint.print_bold(f"========= Rate-dependent regime ==========")
            regime = 'rate-dependent'
        else: 
            ColorPrint.print_bold(f"********* Rate-IN-dependent regime *********")
            regime = 'rate-independent'
        ...
        history_data["regime"].append(regime)
        history_data["reduced_norms"].append(_newton_norm_r)
...
def update_loads(u_, t):
    u_.interpolate(lambda x: (t * np.ones_like(x[0]),  np.zeros_like(x[1])))
    u_.vector.ghostUpdate(addv=PETSc.InsertMode.INSERT,
                            mode=PETSc.ScatterMode.FORWARD)

def setup_ctx(parameters, storage, comm):
    signature = hashlib.md5(str(parameters).encode('utf-8')).hexdigest()
    
    outdir = "output"
    if storage is None:
        prefix = os.path.join(outdir, __file__, signature)
    else:
        prefix = storage
...
def load_parameters(file_path):
    ...
    parameters["model"]["viscous_eps"] = 0
...
if __name__ == "__main__":
    ...
    _storage = f"output/test_viscous_relaxation_slowtime/{signature}"
    ColorPrint.print_bold(f"===================-{_storage}-=================")
    # __import__('pdb').set_trace()
    test_viscous(parameters, _storage)
    ColorPrint.print_bold(f"===================-{signature}-=================")
    ColorPrint.print_bold(f"===================-{_storage}-=================")

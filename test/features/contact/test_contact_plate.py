...
def create_arc_ring_mesh(inner_radius, outer_radius, angle=180, lc=0.1):
    ...
    msh_file = BASE_DIR / "arc_ring.msh"
...
if __name__ == "__main__":
    ...
    with XDMFFile(MPI.COMM_WORLD, str(OUTPUT_DIR / "contact_plate.xdmf"), "w") as xdmf:
    ...
    body_f = dolfinx.fem.Constant(mesh, np.array([0.0, -0.10], dtype=PETSc.ScalarType))
    ...
    for it, t in enumerate(loads):
        u_ub.interpolate(
            lambda x: np.stack(
                [np.full_like(x[0], np.inf), np.full_like(x[1], g_t - np.abs(t) - x[1])]
            )
        )
        ...
        with XDMFFile(MPI.COMM_WORLD, str(OUTPUT_DIR / "contact_plate.xdmf"), "a") as xdmf:

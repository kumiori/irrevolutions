...
def sandbox():
    ...
    extension.scatter(
        vr, x, addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.FORWARD
    )
    ...
    x.zeroEntries()
    extension.scatter(
        vr, x, addv=PETSc.InsertMode.INSERT, mode=PETSc.ScatterMode.REVERSE
    )
    ...
    comm.Barrier()
    _logger.info(f"The local vectors")
    _logger.debug(f"{__log_incipit} Local data of the subvector v_u: {v_u}")
    _logger.debug(f"{__log_incipit} Local data of the subvector v_alpha: {v_alpha}")

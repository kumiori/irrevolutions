...
def plot_fills(ax, ell, tc):
    ax.add_patch(
        patches.Rectangle((0, 0), 1, 10, facecolor=elastic, fill=True, alpha=0.3)
    )
    ax.add_patch(
        patches.Rectangle(
            (tc, 0), t_bif(ell) - 1, 10, facecolor=homogen, fill=True, alpha=0.3
        )
    )
    ax.add_patch(
        patches.Rectangle(
            (t_bif(ell), 0),
            t_stab(ell) - t_bif(ell),
            10,
            facecolor="w",
            fill=True,
            alpha=0.3,
    ...
    ax.add_patch(
        patches.Rectangle(
            (t_stab(ell), 0), 10, 10, facecolor=localis, fill=True, alpha=0.3
        )
    )
    return ax
...
def plot_sigmaeps(params, dataf, tc):
    ...
    # dataf['load'].values*pow(dataf['S(alpha)'].values, -1), marker='o',
    # label='$$\sigma$$')
    ...
    # dataf['load'].values*dataf['A(alpha)'].values*E0/Ly, marker='o',
    # label='$$\sigma$$')
...
def plot_stability(prefix, tol=1e-5):
    ...
    for subdir, dirs, files in os.walk(prefix):
        ...
        with open(subdir + "/time_data.json") as f:
            ...
            mineig = [
                min(eigs) if isinstance(eigs, (list,)) else 100 for eigs in df["eigs"]
            ]
            ...
            # plt.plot(loads, [1/ell]*len(loads), c='C0', marker='.')
            plt.plot(
                loads[elasticloads[-1] : :],
                [1 / ell] * len(loads[elasticloads[-1] : :]),
                c=homogen,
                marker=".",
            )
            plt.plot(
                loads[elasticloads],
                [1 / ell] * len(loads[elasticloads]),
                c=elastic,
                marker=".",
            )
    ...
    q = 2
    coeff_sta = 2.0 * np.pi * q / (q + 1) ** (3.0 / 2.0) * np.sqrt(2)
    coeff_bif = coeff_sta * (q + 1) / (2.0 * q)
    loads = np.linspace(1.0, 10.0, 100)

    ax = plt.gca()
    # ax.plot(loads, [2.*2.*np.pi*q/(q+1)**(3./2.)*np.sqrt(2)/i for i in
    # loads], lw=3, c='k's)
    ax.plot(loads, [coeff_sta / i for i in loads], "-", c="k", label="$$t_s(L/\\ell)$$")
    ax.plot(
        loads, [coeff_bif / i for i in loads], "-.", c="k", label="$$t_b(L/\\ell)$$"
    )
    # plt.axvline(1.0, c='k', lw=1)

    ax.fill_betweenx(
        [coeff_sta / i for i in loads], loads, 20.0, alpha=0.3, facecolor=localis
    )
    # ax.fill_betweenx([coeff_bif/i for i in loads], 0, loads, alpha=.3, facecolor='C1')
    ax.fill_betweenx(
        [coeff_bif / i for i in loads], 1, loads, alpha=0.3, facecolor=homogen
    )
    ...
    ax.add_patch(
        patches.Rectangle(
            (1, coeff_sta), 10, 10, facecolor=localis, fill=True, alpha=0.3
        )
    )
    ...
    plt.xlabel("$t$")
    plt.ylabel("$$L/\\ell$$")
    plt.ylim(0.0, 1.5 * coeff_sta)
    plt.xlim(0.0, max(loads))

    ax.set_yticks([0, 1, 1 / 0.5, 1 / 0.25, coeff_sta, coeff_bif])
...
def _plot_spectrum(data):
    ...
    _lambda_0 = [np.nan if isinstance(a, list) else a for a in data["cone-eig"]]
...
def plot_operator_spectrum(data, parameters):
    ...
    load_steps_all = np.concatenate(
        [
            np.full_like(eigenvalues, load_step)
            for load_step, eigenvalues in zip(data["load"], data["eigs_ball"])
        ]
    )

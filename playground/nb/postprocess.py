...
def plot_stability(prefix, tol=1e-5):
    ...
    for subdir, dirs, files in os.walk(prefix):
        ...
        with open(subdir + "/time_data.json") as f:
            ...
            mineig = [min(eigs) if isinstance(eigs, (list,))
                      else 100 for eigs in df['eigs']]
            ...
            # plt.plot(loads, [1/ell]*len(loads), c='C0', marker='.')
            plt.plot(loads[elasticloads[-1]::], [1 / ell] *
                     len(loads[elasticloads[-1]::]), c=homogen, marker='.')
            plt.plot(loads[elasticloads], [1 / ell] *
                     len(loads[elasticloads]), c=elastic, marker='.')
    ...
    plt.plot((20, 20), (20, 20), ls='', c='k',
             marker='X', label='incr. unstable')
    plt.plot((20, 20), (20, 20), ls='', c=elastic, marker='.', label='elastic')
    plt.plot((20, 20), (20, 20), ls='', c=homogen,
             marker='.', label='incr. \\& state stable ')
    ...
    # loads], lw=3, c='k's)
    ax.plot(loads, [coeff_sta / i for i in loads],
            '-', c='k', label='$$t_s(L/\\ell)$$')
    ax.plot(loads, [coeff_bif / i for i in loads],
            '-.', c='k', label='$$t_b(L/\\ell)$$')
    # plt.axvline(1.0, c='k', lw=1)

    ax.fill_betweenx([coeff_sta / i for i in loads], loads,
                     20., alpha=.3, facecolor=localis)
    # ax.fill_betweenx([coeff_bif/i for i in loads], 0, loads, alpha=.3, facecolor='C1')
    ax.fill_betweenx([coeff_bif / i for i in loads], 1,
                     loads, alpha=.3, facecolor=homogen)
...
def _plot_spectrum(data):
    def _stab_cnd(data): return [
        0 if data["cone-stable"][i] == True else 1 for i in range(len(data))]
    # _uniq_cnd = [.3 if d[0]>0 else .7 for d in data['eigs']]
    """docstring for plotSpaceVsCone"""
    figure, axis = plt.subplots(1, 1)
    _lambda_0 = [
        np.nan if isinstance(
            a, list) else a for a in data["cone-eig"]]

...
def run_computation(parameters, storage=None):
    ...
    while True:
        ...
        with dolfinx.common.Timer(f"~Postprocessing and Vis") as timer:
            ...
            magnitude_expr = dolfinx.fem.Expression(
                u[0], V_alpha.element.interpolation_points()
            )
            ...
            if comm.rank == 0:
                ...
                _plt = plot_vector(u, plotter, subplot=(0, 1), scale=-0.8)
    ...
    print(
        pd.DataFrame(history_data).drop(
            columns=["cone_data", "equilibrium_data", "eigs_cone", "stable", "inertia"]
        )
    )

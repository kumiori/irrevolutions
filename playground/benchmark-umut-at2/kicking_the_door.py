...
def test_linsearch(parameters, storage):
    ...
    if storage is None:
        prefix = os.path.join(outdir, "kick-at2-thinfilm-1d", signature)
    else:
        prefix = storage
    ...
    zero_u = Function(V_u, name="BoundaryDisplacement")
    alpha = Function(V_alpha, name="Damage")
    β = Function(V_alpha, name="Damageperturbation")
    zero_alpha = Function(V_alpha, name="DamageBoundaryField")
    ...
    for i_t, t in enumerate(loads):
        ...
        rate_12_norm = equilibrium.scaled_rate_norm(alpha, parameters)
        urate_12_norm = equilibrium.unscaled_rate_norm(alpha)
